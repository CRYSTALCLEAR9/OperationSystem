...
static volatile int g_smode_init_ready[NCPU] = { 0 };
...
int s_start(void) {
  ...
  write_csr(satp, 0);
       ...
       // Only one hart initializes global memory manager and kernel page table.
       if (hartid == 0) {
       ...
       // Ensure all harts observe initialized PMM/page table before enabling paging.
       g_smode_init_ready[hartid] = 1;
       wait_all_harts_ready(g_smode_init_ready);
